...
// 独立计数器，为每个相机维护从0开始的序号
static uint64_t g_cam_local_counters[CAM_COUNT] = {0};
...
void MyStreamCallBack(HIMAGE h_img, HSUBDEVICE h_dev, unsigned int streamIndex, unsigned int event, void* context) {
    ...
    if (s_master_auto_sync_enabled && cam_index == s_master_auto_cam_idx) SyncSlaveExposureFromMaster();

    // 独立计数器自增 (每个相机从0开始算第一帧)
    uint64_t seq = g_cam_local_counters[cam_index]++;
    
    // 获取微秒级系统时间
    struct timeval tv;
    gettimeofday(&tv, NULL);
    uint64_t current_ts_us = (uint64_t)tv.tv_sec * 1000000ULL + (uint64_t)tv.tv_usec;

    pthread_mutex_lock(&g_sync_box.lock);

    // 追随最大序号算法：若来的帧序号比全局正在凑的序号新
    if (seq > g_sync_box.target_seq) {
        for (int i = 0; i < CAM_COUNT; ++i) {
            if (g_sync_box.images[i] != NULL) {
                g_Image_Recycle(g_sync_box.images[i]);
                g_sync_box.images[i] = NULL;
        ...
        g_sync_box.ready_mask = 0;
        g_sync_box.target_seq = seq;
        g_sync_box.capture_timestamp_us = current_ts_us;
    } 
    else if (seq < g_sync_box.target_seq) {
        pthread_mutex_unlock(&g_sync_box.lock);
        g_Image_Recycle(h_img);
        return;
    }

    if (g_sync_box.ready_mask == 0) g_sync_box.capture_timestamp_us = current_ts_us;

    if (g_sync_box.images[cam_index]) g_Image_Recycle(g_sync_box.images[cam_index]);

    g_sync_box.images[cam_index] = h_img;
    g_sync_box.block_ids[cam_index] = imageInfo.blockId;
    g_sync_box.ready_mask |= (1 << cam_index);

    if (g_sync_box.ready_mask == 15) pthread_cond_signal(&g_sync_box.cond);
